<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="代理地址" width="300px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.address }}</span>
        </template>
      </el-table-column>
      <el-table-column label="代理QQ" align="center" width="200px">
        <template slot-scope="{row}">
          <span>{{ row.qq }}</span>
        </template>
      </el-table-column>
      <el-table-column label="代理状态" align="center" width="170">
        <template slot-scope="{row}">
...
<script>
import { fetchList, fetchPv, createArticle, updateArticle, updateStatus } from '@/api/article'
